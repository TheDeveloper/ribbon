...
Ribbon.prototype.setClientFnStatus = function(name, status){
  this.clientFnStatus[name] = status;
};

Ribbon.prototype.clientFn = function(name, done){
  var fn;

  if (typeof (fn = this.clientFns[name]) !== 'function') return self.debug('No client function for %s action', name);

  this.setClientFnStatus(true);
  this.stats[name] += 1;
...
Ribbon.prototype.startUp = function(cb){
  ...
  var done = function(err, client){
    ...
    return cb(err, client || self.client());
