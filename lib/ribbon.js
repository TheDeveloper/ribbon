...
Ribbon.prototype.runAction = function(name, cb){
  ...
  if(typeof this.opts.actionTimeout == 'number'){
    var timerFunc = setTimeout;
    if(this.opts.actionTimeout === 0) timerFunc = process.nextTick;
    var hitTimeout = function(){
      self.emit('timeout:'+name);
      cb('timeout');
      cb = function(){};
    };
    timeout = timerFunc(hitTimeout, this.opts.actionTimeout);
  }
...
Ribbon.prototype.restart = function(cb){
  ...
  self.restartDelay = Math.floor(self.restartDelay * self.opts.backoffCoefficient);
};
 ...
 */
Ribbon.prototype.dropped = function(err){
  var self = this;
